...
export const handleImageError = (e, imageUrl) => {
  ...
  if (!fallback) {
    ...
    if (parent) {
      ...
      fallbackDiv.className = 'w-full h-full flex items-center justify-center bg-hitman-700 text-hitman-400';
      fallbackDiv.innerHTML = '<svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>';
...
export const isValidImageUrl = (imageUrl) => {
  ...
  try {
    const url = new URL(imageUrl.startsWith('http') ? imageUrl : `http://localhost:5000${imageUrl}`);
